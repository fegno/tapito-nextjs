...
export default function SchemeGeneratorPage() {
  return (
    <div className="relative min-h-screen overflow-x-hidden bg-white pt-24 text-slate-900 selection:bg-purple-100 selection:text-purple-900 font-inter">
      ...
      <section className="bg-gradient-to-br from-purple-50 via-white to-pink-50 py-24 overflow-hidden relative group font-inter">
        <Container>
          <motion.div variants={revealVariant} initial="initial" whileInView="whileInView" viewport={{ once: true, amount: 0.4 }} className="rounded-[3.5rem] border border-purple-200 bg-white p-12 text-center shadow-[0_60px_160px_-80px_rgba(124,58,237,0.5)] lg:p-24 relative overflow-hidden">
            ...
            <motion.div whileHover={{ scale: 1.25, rotate: 180 }} transition={{ duration: 1 }} className="mx-auto flex h-24 w-24 items-center justify-center rounded-[2.5rem] bg-purple-600 text-white shadow-2xl shadow-purple-500/40 relative z-10"><Sparkles size={36} /></motion.div>
            <h2 className="mt-10 text-4xl font-black tracking-[-0.05em] text-slate-950 lg:text-6xl relative z-10 uppercase font-outfit">Optimize the business outcome, not just the discount.</h2>
            <p className="mx-auto mt-10 max-w-4xl text-xl font-medium leading-10 text-slate-600 relative z-10 opacity-80 group-hover:opacity-100 italic font-inter">Traditional discount planning starts with an offer. Tapito starts with inventory tokens — stock pressure, customer response, and margin exposure — then works backward to the healthiest possible scheme suggestion.</p>
          </motion.div>
        </Container>
      </section>
