...
export default function AIAssistantPage() {
  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden bg-white pt-24 text-slate-900 selection:bg-blue-100 selection:text-blue-900 font-inter">
      <InteractiveGrid />
      <div className="pointer-events-none absolute inset-x-0 top-0 h-[400px] lg:h-[540px] bg-[radial-gradient(circle_at_top,rgba(59,130,246,0.16),transparent_38%),radial-gradient(circle_at_top_right,rgba(9,53,140,0.13),transparent_25%)]" />
            ...
            <div>
              ...
              <motion.div {...fadeUp(0.24)} className="mt-10 flex flex-wrap items-center gap-4">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-xs font-black uppercase tracking-widest text-blue-600">Voice • Chat • Analysis</span>
                </div>
              </motion.div>
            </div>
            ...
            <motion.div variants={revealVariant} initial="initial" whileInView="whileInView" viewport={{ once: true, amount: 0.3 }} className="rounded-[2rem] lg:rounded-[3rem] border border-slate-800 bg-slate-950 p-6 shadow-[0_50px_130px_-60px_rgba(15,23,42,0.95)] relative overflow-hidden group">
              ...
              <div className="flex items-center justify-between rounded-2xl border border-white/10 bg-white/5 px-5 py-4 relative z-10 transition-colors group-hover:bg-white/10">
                <div className="flex items-center gap-4">
                  <motion.div whileHover={{ scale: 1.1, rotate: 5 }} className="flex h-12 w-12 items-center justify-center rounded-xl bg-blue-600 text-white shadow-lg shadow-blue-500/30"><Bot size={22} /></motion.div>
                  <div>
              ...
              <motion.div variants={staggerContainer} initial="initial" whileInView="whileInView" className="mt-8 space-y-5 relative z-10">
                ...
                <motion.div variants={bubbleVariant("right")} className="ml-auto max-w-[80%] rounded-[2rem] rounded-tr-md bg-gradient-to-r from-blue-600 to-indigo-600 p-5 text-base font-black leading-relaxed text-white shadow-xl shadow-blue-500/20">
                ...
                <motion.div variants={bubbleVariant("left")} className="max-w-[90%] rounded-[2rem] rounded-tl-md bg-white/8 p-6 border border-white/5 shadow-inner">
                  ...
                  <motion.div initial={{ y: 20, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} transition={{ delay: 1 }} className="mt-6 rounded-2xl border border-white/10 bg-slate-900/80 p-5 group/card hover:bg-slate-800 transition-colors">
                    <p className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-400/80">Contextual Insight Card</p>
                    ...
                    <div className="mt-3 h-2.5 rounded-full bg-white/10 overflow-hidden">
                      <motion.div initial={{ width: 0 }} whileInView={{ width: "72%" }} transition={{ duration: 1.5, delay: 1.2, ease: "circOut" }} className="h-full rounded-full bg-gradient-to-r from-emerald-500 via-blue-500 to-indigo-500" />
                    </div>
                  </motion.div>
                </motion.div>
              </motion.div>
      ...
      <section className="border-y border-slate-200 bg-slate-50/80 py-16 4xl:py-24">
        <Container>
          <motion.div variants={staggerContainer} initial="initial" whileInView="whileInView" viewport={{ once: true }} className="grid gap-6 lg:grid-cols-3">
            {[["Business problem", "Critical answers stay trapped behind dashboards, filters, and people who know how to navigate them.", "text-rose-500"], ["What Tapito detects", "Tapito detects intent, business context, and follow-up meaning inside voice or chat questions.", "text-blue-600"], ["What action it enables", "Anyone can get live business intelligence instantly, even while moving through the store floor.", "text-emerald-600"]].map(([title, text, tone], index) => (
              <motion.div key={title} {...fadeUp(index * 0.1)} {...cardHover} className={`rounded-[2.5rem] border bg-white p-10 group transition-all ${index === 1 ? "border-blue-200 shadow-[0_40px_90px_-50px_rgba(37,99,235,0.5)]" : "border-slate-200 hover:border-blue-200"}`}>
              ...
              {[["01", "Ask naturally", "Speak or type the same way you would ask an analyst or lead."], ["02", "Interpret context", "Tapito understands retail language, time windows, and follow-up chains."], ["03", "Respond with move", "Answers include the metric, the shift, and the suggested next move."]].map(([step, title, text], index) => (
                <motion.div key={step} {...fadeUp(index * 0.15)} {...cardHover} className="rounded-[2rem] border border-slate-200 bg-white p-8 group hover:bg-slate-50 transition-colors h-full">
                  <div className="flex items-start gap-6">
                    <motion.div whileHover={{ scale: 1.1, rotate: 10 }} className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-slate-950 text-base font-black text-white group-hover:bg-blue-600 transition-colors shadow-lg">{step}</motion.div>
                    <div>
                      <h3 className="text-2xl font-black text-slate-950 group-hover:text-blue-700 transition-colors">{title}</h3>
            ...
            <motion.div variants={revealVariant} initial="initial" whileInView="whileInView" viewport={{ once: true, amount: 0.16 }} className="rounded-[3.5rem] border border-blue-200 bg-gradient-to-br from-white via-blue-50/30 to-indigo-50/50 p-10 lg:p-14 relative group">
              ...
              <div className="grid gap-6 md:grid-cols-2 relative z-10">
                <motion.div {...fadeUp(0.6)} {...cardHover} className="rounded-[2rem] border border-slate-800 bg-slate-950 p-8 shadow-xl group/card">
                   ...
                   <div className="mt-8 px-4 py-5 rounded-2xl bg-white/5 border border-white/5 relative overflow-hidden text-white/90 font-black group-hover/card:border-blue-500/30 transition-colors">
                     "What changed in Central Hub?"
                     <motion.div animate={{ x: ["-100%", "100%"] }} transition={{ duration: 2, repeat: Infinity, ease: "linear" }} className="absolute bottom-0 left-0 w-1/3 h-[2px] bg-gradient-to-r from-transparent via-blue-500 to-transparent" />
                   </div>
                </motion.div>
                <motion.div {...fadeUp(0.7)} {...cardHover} className="luminous-card luminous-surface-strong rounded-[2rem] border border-blue-100 bg-white p-8 shadow-sm group/card">
                   ...
                   <div className="mt-8 space-y-3 opacity-80 group-hover/card:opacity-100 transition-opacity">
                     {["GP margin trend...", "Comparison vs Last Yr...", "Specific SKU impact..."].map(q => (
                       <div key={q} className="px-4 py-3 rounded-xl bg-blue-50 border border-blue-100/50 text-xs font-bold text-slate-600 hover:bg-white transition-colors cursor-pointer">{q}</div>
                     ))}
                   </div>
                </motion.div>
              </div>
          ...
          <motion.div variants={staggerContainer} initial="initial" whileInView="whileInView" viewport={{ once: true }} className="grid gap-8 md:grid-cols-3">
            {[["<2s", "Decision speed", "Average response for complex retail queries"], ["+100%", "Data adoption", "Teams using data to make floor-level decisions"], ["-90%", "Reporting burden", "Zero time spent building manual spreadsheets"]].map(([value, label, note], index) => (
              <motion.div key={label} {...fadeUp(index * 0.15)} {...cardHover} className="rounded-[3rem] border border-slate-200 bg-white p-12 text-center group hover:bg-blue-50/30 transition-all shadow-sm">
                <motion.p animate={{ scale: [1, 1.05, 1] }} transition={{ duration: 3, repeat: Infinity }} className="text-6xl font-black tracking-tight text-blue-700 transition-transform group-hover:scale-110">{value}</motion.p>
        ...
        <Container>
          ...
          <div className="grid overflow-hidden rounded-[3.5rem] border border-slate-200 shadow-[0_50px_140px_-60px_rgba(15,23,42,0.4)] md:grid-cols-2 relative h-full">
            ...
            <motion.div initial={{ x: 100, opacity: 0 }} whileInView={{ x: 0, opacity: 1 }} viewport={{ once: true }} transition={{ duration: 1 }} className="bg-slate-950 p-12 lg:p-20 text-white relative h-full group">
              ...
              <p className="text-xs font-black uppercase tracking-[0.3em] text-blue-300 relative z-10">After Tapito Assistant</p>
