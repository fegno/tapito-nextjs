...
export default function AIAssistantPage() {
  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden bg-white pt-24 text-slate-900 selection:bg-[#09358c]/10 selection:text-[#09358c] font-inter">
      <InteractiveGrid />
      <div className="pointer-events-none absolute inset-x-0 top-0 h-[400px] lg:h-[540px] bg-[radial-gradient(circle_at_top,rgba(9,53,140,0.12),transparent_38%),radial-gradient(circle_at_top_right,rgba(5,160,236,0.1),transparent_25%)]" />
            ...
            <div>
              ...
              <motion.div {...fadeUp(0.24)} className="mt-10 flex flex-wrap items-center gap-4">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-xs font-black uppercase tracking-widest text-[#05a0ec]">Voice • Chat • Analysis</span>
                </div>
              </motion.div>
            </div>

            <motion.div variants={revealVariant} initial="initial" whileInView="whileInView" viewport={{ once: true, amount: 0.3 }} className="rounded-[2rem] lg:rounded-[3rem] border border-slate-800 bg-slate-950 p-6 shadow-[0_50px_130px_-60px_rgba(9,53,140,0.5)] relative overflow-hidden group">
              ...
              <div className="flex items-center justify-between rounded-2xl border border-white/10 bg-white/5 px-5 py-4 relative z-10 transition-colors group-hover:bg-white/10">
                <div className="flex items-center gap-4">
                  <motion.div whileHover={{ scale: 1.1, rotate: 5 }} className="flex h-12 w-12 items-center justify-center rounded-xl bg-[#09358c] text-white shadow-lg shadow-[#09358c]/30"><Bot size={22} /></motion.div>
                  <div>
              ...
              <motion.div variants={staggerContainer} initial="initial" whileInView="whileInView" className="mt-8 space-y-5 relative z-10">
                ...
                <motion.div variants={bubbleVariant("left")} className="max-w-[90%] rounded-[2rem] rounded-tl-md bg-white/8 p-6 border border-white/5 shadow-inner">
                  ...
                  <motion.div initial={{ y: 20, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} transition={{ delay: 1 }} className="mt-6 rounded-2xl border border-white/10 bg-slate-900/80 p-5 group/card hover:bg-slate-800 transition-colors">
                    <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#05a0ec]/80">Contextual Insight Card</p>
                    ...
                    <div className="mt-3 h-2.5 rounded-full bg-white/10 overflow-hidden">
                      <motion.div initial={{ width: 0 }} whileInView={{ width: "72%" }} transition={{ duration: 1.5, delay: 1.2, ease: "circOut" }} className="h-full rounded-full bg-gradient-to-r from-[#05a0ec] via-[#09358c] to-[#05a0ec]" />
                    </div>
                  </motion.div>
                </motion.div>
              </motion.div>
      ...
      <section className="border-y border-slate-200 bg-slate-50/80 py-16 4xl:py-24">
        <Container>
          <motion.div variants={staggerContainer} initial="initial" whileInView="whileInView" viewport={{ once: true }} className="grid gap-6 lg:grid-cols-3">
            {[["Business problem", "Critical answers stay trapped behind dashboards, filters, and people who know how to navigate them.", "text-rose-500"], ["What Tapito detects", "Tapito detects intent, business context, and follow-up meaning inside voice or chat questions.", "text-[#09358c]"], ["What action it enables", "Anyone can get live business intelligence instantly, even while moving through the store floor.", "text-emerald-600"]].map(([title, text, tone], index) => (
              <motion.div key={title} {...fadeUp(index * 0.1)} {...cardHover} className={`rounded-[2.5rem] border bg-white p-10 group transition-all ${index === 1 ? "border-[#09358c]/20 shadow-[0_40px_90px_-50px_rgba(9,53,140,0.3)]" : "border-slate-200 hover:border-[#09358c]/20"}`}>
      ...
      <section className="py-24 relative overflow-hidden font-inter">
        ...
        <Container>
          ...
          <div className="grid gap-12 lg:grid-cols-[0.85fr_1.15fr] items-center">
              ...
              {[["01", "Ask naturally", "Speak or type the same way you would ask an analyst or lead."], ["02", "Interpret context", "Tapito understands retail language, time windows, and follow-up chains."], ["03", "Respond with move", "Answers include the metric, the shift, and the suggested next move."]].map(([step, title, text], index) => (
                <motion.div key={step} {...fadeUp(index * 0.15)} {...cardHover} className="rounded-[2rem] border border-slate-200 bg-white p-8 group hover:bg-slate-50 transition-colors h-full">
                  <div className="flex items-start gap-6">
                    <motion.div whileHover={{ scale: 1.1, rotate: 10 }} className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-slate-950 text-base font-black text-white group-hover:bg-[#09358c] transition-colors shadow-lg">{step}</motion.div>
                    <div>
                      <h3 className="text-2xl font-black text-slate-950 group-hover:text-[#09358c] transition-colors">{title}</h3>
            ...
            <motion.div variants={revealVariant} initial="initial" whileInView="whileInView" viewport={{ once: true, amount: 0.16 }} className="rounded-[3.5rem] border border-[#09358c]/10 bg-gradient-to-br from-white via-[#09358c]/2 to-[#05a0ec]/5 p-10 lg:p-14 relative group">
              ...
              <div className="grid gap-6 md:grid-cols-2 relative z-10">
                <motion.div {...fadeUp(0.6)} {...cardHover} className="rounded-[2rem] border border-slate-800 bg-slate-950 p-8 shadow-xl group/card">
                   <div className="flex items-center justify-between">
                     <p className="text-xs font-black uppercase tracking-[0.2em] text-[#05a0ec]">Voice layer</p>
                     <Waves size={22} className="text-[#05a0ec] group-hover/card:scale-110 transition-transform" />
                   </div>
                   <div className="mt-8 px-4 py-5 rounded-2xl bg-white/5 border border-white/5 relative overflow-hidden text-white/90 font-black group-hover/card:border-[#09358c]/30 transition-colors">
                     "What changed in Central Hub?"
                     <motion.div animate={{ x: ["-100%", "100%"] }} transition={{ duration: 2, repeat: Infinity, ease: "linear" }} className="absolute bottom-0 left-0 w-1/3 h-[2px] bg-gradient-to-r from-transparent via-[#09358c] to-transparent" />
                   </div>
                </motion.div>
                <motion.div {...fadeUp(0.7)} {...cardHover} className="luminous-card luminous-surface-strong rounded-[2rem] border border-[#09358c]/10 bg-white p-8 shadow-sm group/card">
                   ...
                   <div className="mt-8 space-y-3 opacity-80 group-hover/card:opacity-100 transition-opacity">
                     {["GP margin trend...", "Comparison vs Last Yr...", "Specific SKU impact..."].map(q => (
                       <div key={q} className="px-4 py-3 rounded-xl bg-[#09358c]/5 border border-[#09358c]/10 text-xs font-bold text-slate-600 hover:bg-white transition-colors cursor-pointer">{q}</div>
                     ))}
                   </div>
                </motion.div>
              </div>
