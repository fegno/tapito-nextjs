          ...
          <div className="grid items-center gap-8 lg:gap-16 lg:grid-cols-[1.05fr_0.95fr]">
            <div>
              ...
              <motion.div variants={staggerContainer} initial="initial" whileInView="whileInView" viewport={{ once: true }} className="mt-10 grid gap-4 sm:grid-cols-2">
                {metrics.map((metric, index) => (
                  <motion.div key={metric.label} {...fadeUp(index * 0.05)} {...cardHover} className="group relative overflow-hidden rounded-3xl border border-slate-200 bg-white/90 p-5 shadow-[0_20px_60px_-45px_rgba(9,53,140,0.2)] backdrop-blur">
                    <div className="absolute inset-0 bg-gradient-to-br from-[#09358c]/5 to-transparent opacity-0 transition-opacity group-hover:opacity-100" />
                    ...
                    <div className="relative mt-3 flex items-end justify-between gap-4">
                      <span className="text-3xl font-black tracking-tight text-slate-950 transition-colors group-hover:text-[#09358c]">{metric.value}</span>
                      <span className="text-xs font-bold text-[#05a0ec]">{metric.note}</span>
                    </div>
                  </motion.div>
                ))}
              </motion.div>
            ...
            <motion.div variants={revealVariant} initial="initial" whileInView="whileInView" viewport={{ once: true, amount: 0.25 }} className="rounded-[2rem] lg:rounded-[2.5rem] border border-slate-200 bg-slate-950 p-6 shadow-[0_50px_120px_-40px_rgba(15,23,42,0.9)]">
              <div className="mb-5 flex items-center justify-between rounded-4xl border border-white/10 bg-white/5 px-4 py-3">
                <div>
                  <p className="text-xs font-black uppercase tracking-[0.22em] text-[#05a0ec]">Tapito Command Center</p>
                ...
                <div className="rounded-[1.5rem] border border-white/10 bg-white/5 p-4 md:col-span-2">
                  ...
                  <div className="flex h-40 items-end gap-2">
                    {[32, 48, 42, 66, 54, 78, 64, 88, 72, 96, 80, 92].map((bar, index) => (
                      <motion.div key={index} initial={{ height: 0, opacity: 0 }} whileInView={{ height: `${bar}%`, opacity: 1 }} viewport={{ once: true }} transition={{ duration: 1, delay: 0.5 + index * 0.05, ease: "easeOut" }} className="flex-1 rounded-t-4xl bg-gradient-to-t from-[#09358c]/40 via-[#09358c]/80 to-[#05a0ec]" />
                    ))}
                  </div>
                </div>
                    ...
                    {[["Sector 12", "$17,200", "100%"], ["Industrial Park", "$14,800", "84%"], ["Downtown Hub", "$11,400", "68%"]].map(([name, value, width]) => (
                      <div key={name}>
                        ...
                        <div className="h-2 rounded-full bg-white/10">
                          <motion.div initial={{ width: 0 }} whileInView={{ width }} viewport={{ once: true }} transition={{ duration: 1.2, delay: 1, ease: "easeOut" }} className="h-2 rounded-full bg-gradient-to-r from-[#09358c] to-[#05a0ec]" />
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {[["01", "Sync every outlet", "POS, billing, inventory, and customer movement land in one live operating layer."], ["02", "Surface what matters", "Tapito highlights branch anomalies, category momentum, and revenue shifts automatically."], ["03", "Drive leadership action", "Managers drill into the exact branch or category that needs intervention next."]].map(([step, title, text], index) => (
                <motion.div key={step} {...fadeUp(index * 0.1)} {...cardHover} className="rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-[0_20px_60px_-45px_rgba(15,23,42,0.35)] group">
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <h3 className="text-xl font-black text-slate-950 transition-colors group-hover:text-[#09358c]">{title}</h3>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div className="rounded-[1.5rem] border border-[#05a0ec]/10 bg-white p-5">
                  ...
                  <div className="mt-4 space-y-3">
                    {[["High attention", "3 stores", "bg-rose-500"], ["Stable growth", "12 stores", "bg-emerald-500"], ["Upside potential", "9 stores", "bg-[#09358c]"]].map(([label, value, tone]) => (
