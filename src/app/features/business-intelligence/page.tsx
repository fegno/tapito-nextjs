...
export default function BusinessIntelligencePage() {
  const feature = featuresData.find(f => f.slug === "business-intelligence");
  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden bg-white pt-24 text-slate-900 selection:bg-blue-100 selection:text-blue-900">
      <InteractiveGrid />
      <div className="pointer-events-none absolute inset-x-0 top-0 h-[400px] lg:h-[520px] bg-[radial-gradient(circle_at_top_right,rgba(59,130,246,0.16),transparent_42%),radial-gradient(circle_at_top_left,rgba(56,189,248,0.14),transparent_30%)]" />
          ...
          <div className="grid items-center gap-8 lg:gap-16 lg:grid-cols-[1.05fr_0.95fr]">
            <div>
              ...
              <motion.div {...fadeUp(0.24)} className="mt-10 flex flex-wrap items-center gap-4">
                <Link href="/enterprise" className="group inline-flex items-center gap-3 rounded-full bg-gradient-to-r from-blue-600 to-indigo-600 px-8 py-4 text-base font-black text-white shadow-[0_20px_60px_-18px_rgba(37,99,235,0.55)] transition-all hover:scale-[1.05] hover:shadow-[0_25px_70px_-15px_rgba(37,99,235,0.65)]">
                ...
                {metrics.map((metric, index) => (
                  <motion.div key={metric.label} {...fadeUp(index * 0.05)} {...cardHover} className="group relative overflow-hidden rounded-3xl border border-slate-200 bg-white/90 p-5 shadow-[0_20px_60px_-45px_rgba(15,23,42,0.45)] backdrop-blur">
                    ...
                    <div className="relative mt-3 flex items-end justify-between gap-4">
                      <span className="text-3xl font-black tracking-tight text-slate-950 transition-colors group-hover:text-blue-700">{metric.value}</span>
                      <span className="text-xs font-bold text-blue-600">{metric.note}</span>
                    </div>
                  </motion.div>
                ))}
              </motion.div>
            ...
            <motion.div variants={revealVariant} initial="initial" whileInView="whileInView" viewport={{ once: true, amount: 0.25 }} className="rounded-[2rem] lg:rounded-[2.5rem] border border-slate-200 bg-slate-950 p-6 shadow-[0_50px_120px_-40px_rgba(15,23,42,0.9)]">
              <div className="mb-5 flex items-center justify-between rounded-4xl border border-white/10 bg-white/5 px-4 py-3">
                <div>
                  <p className="text-xs font-black uppercase tracking-[0.22em] text-blue-300">Tapito Command Center</p>
                ...
                <div className="rounded-[1.5rem] border border-white/10 bg-white/5 p-4 md:col-span-2">
                  ...
                  <div className="flex h-40 items-end gap-2">
                    {[32, 48, 42, 66, 54, 78, 64, 88, 72, 96, 80, 92].map((bar, index) => (
                      <motion.div key={index} initial={{ height: 0, opacity: 0 }} whileInView={{ height: `${bar}%`, opacity: 1 }} viewport={{ once: true }} transition={{ duration: 1, delay: 0.5 + index * 0.05, ease: "easeOut" }} className="flex-1 rounded-t-4xl bg-gradient-to-t from-blue-500/25 via-blue-500/65 to-sky-300/90" />
                    ))}
                  </div>
                </div>
                    ...
                    {[["Sector 12", "Rs 14.2L", "100%"], ["Industrial Park", "Rs 11.8L", "84%"], ["Downtown Hub", "Rs 9.4L", "68%"]].map(([name, value, width]) => (
                      <div key={name}>
                        ...
                        <div className="h-2 rounded-full bg-white/10">
                          <motion.div initial={{ width: 0 }} whileInView={{ width }} viewport={{ once: true }} transition={{ duration: 1.2, delay: 1, ease: "easeOut" }} className="h-2 rounded-full bg-gradient-to-r from-blue-400 to-sky-300" />
                        </div>
                      </div>
                    ))}
                  </div>
      ...
      <section className="py-16 4xl:py-24">
        <Container>
          ...
          <div className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
              ...
              {[ ["01", "Sync every outlet", "POS, billing, inventory, and customer movement land in one live operating layer."], ["02", "Surface what matters", "Tapito highlights branch anomalies, category momentum, and revenue shifts automatically."], ["03", "Drive leadership action", "Managers drill into the exact branch or category that needs intervention next."] ].map(([step, title, text], index) => (
                <motion.div key={step} {...fadeUp(index * 0.1)} {...cardHover} className="rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-[0_20px_60px_-45px_rgba(15,23,42,0.35)] group">
                  <div className="flex items-start gap-4">
                    <motion.div whileHover={{ scale: 1.1, rotate: 10 }} className="flex h-12 w-12 shrink-0 items-center justify-center rounded-4xl bg-slate-950 text-sm font-black text-white transition-colors group-hover:bg-blue-600">{step}</motion.div>
                    <div>
                      <h3 className="text-xl font-black text-slate-950 transition-colors group-hover:text-blue-700">{title}</h3>
            ...
            <motion.div variants={revealVariant} initial="initial" whileInView="whileInView" viewport={{ once: true, amount: 0.16 }} className="rounded-[2.5rem] border border-blue-200 bg-gradient-to-br from-blue-50 via-white to-blue-50 p-6 lg:p-8">
              <div className="grid gap-4 md:grid-cols-2">
                <div className="rounded-[1.5rem] border border-blue-100 bg-white p-5">
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-3">
                    {[["Bills", "12.4k"], ["ABV", "Rs 3.4k"], ["GP", "22.1%"], ["Stock", "Healthy"]].map(([label, value]) => (
                      <motion.div key={label} whileHover={{ y: -5 }} className="luminous-card luminous-surface-strong soft-shine rounded-4xl bg-blue-50 p-3">
                        <p className="text-[10px] font-black uppercase tracking-[0.18em] text-blue-500">{label}</p>
                ...
                <div className="rounded-[1.5rem] border border-blue-100 bg-white p-5">
                  ...
                  <div className="mt-4 space-y-3">
                    {[["High attention", "3 stores", "bg-rose-500"], ["Stable growth", "12 stores", "bg-emerald-500"], ["Upside potential", "9 stores", "bg-blue-500"]].map(([label, value, tone]) => (
                ...
                <div className="rounded-[1.5rem] border border-blue-100 bg-white p-5 md:col-span-2">
                  ...
                  <div className="mt-4 grid gap-3 md:grid-cols-3">
                    {[["Store focus", "Rework assortment in Downtown Hub"], ["Category action", "Push Fashion in Central Hub this weekend"], ["Leadership note", "Margin expansion holding above target"]].map(([label, text], index) => (
                      <motion.div key={label} whileHover={{ scale: 1.05 }} className="luminous-card-dark rounded-4xl border border-slate-200 bg-slate-950 p-4 text-white">
                        <p className="text-[10px] font-black uppercase tracking-[0.16em] text-blue-300">{label}</p>
      ...
      <section className="py-16 4xl:py-24 relative overflow-hidden">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full bg-[radial-gradient(circle,rgba(59,130,246,0.05),transparent_70%)] pointer-events-none" />
        <Container>
          ...
          <motion.div variants={staggerContainer} initial="initial" whileInView="whileInView" viewport={{ once: true }} className="grid gap-6 md:grid-cols-3">
            {[["+300%", "Reporting speed", "Less waiting for consolidated files"], ["-68%", "Response time", "Managers act while issues are fresh"], ["360deg", "Business visibility", "Branch, category, and revenue in one flow"]].map(([value, label, note], index) => (
              <motion.div key={label} {...fadeUp(index * 0.1)} {...cardHover} className="rounded-[2.5rem] border border-slate-200 bg-gradient-to-b from-white to-blue-50/30 p-10 text-center group">
                <motion.p initial={{ scale: 0.5, opacity: 0 }} whileInView={{ scale: 1, opacity: 1 }} transition={{ duration: 0.6, delay: 0.3 + index * 0.1 }} className="text-6xl font-black tracking-tight text-blue-700 transition-transform group-hover:scale-110">{value}</motion.p>
        ...
        <Container>
          ...
          <div className="grid overflow-hidden rounded-[3rem] border border-slate-200 shadow-[0_45px_120px_-60px_rgba(15,23,42,0.4)] md:grid-cols-2">
            ...
            <motion.div initial={{ x: 50, opacity: 0 }} whileInView={{ x: 0, opacity: 1 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="bg-slate-950 p-12 lg:p-16 text-white relative">
              ...
              <p className="text-sm font-black uppercase tracking-[0.22em] text-blue-300 relative z-10">After Tapito</p>
