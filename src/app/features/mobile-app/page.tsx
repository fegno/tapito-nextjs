  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden bg-white pt-24 text-slate-900 selection:bg-blue-100 selection:text-blue-900 font-inter">
      ...
      <section className="relative py-12 md:py-20 4xl:py-28">
        <Container>
          <div className="grid gap-10 lg:gap-20 lg:grid-cols-[0.92fr_1.08fr]">
            <div className="text-reveal">
              ...
              <motion.div {...fadeUp(0.24)} className="mt-10 flex flex-wrap items-center gap-6">
                <Link href="/enterprise" className="group inline-flex items-center gap-3 rounded-full bg-gradient-to-r from-blue-600 to-indigo-600 px-10 py-5 text-base font-black text-white shadow-[0_25px_60px_-18px_rgba(124,58,237,0.6)] transition-all hover:scale-[1.05] hover:shadow-[0_30px_70px_-15px_rgba(124,58,237,0.7)]">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-xs font-black uppercase tracking-widest text-blue-600">iOS & Android Natives</span>
                </div>
              </motion.div>
            </div>
            ...
            <motion.div variants={revealVariant} initial="initial" whileInView="whileInView" viewport={{ once: true, amount: 0.3 }} className="relative flex justify-center group">
              <div className="absolute h-[420px] w-[420px] rounded-full bg-blue-200/30 blur-[130px] -z-10 animate-pulse" />
              <motion.div whileHover={{ rotateY: -10, rotateX: 5 }} transition={{ duration: 0.6 }} className="relative max-w-[540px] rounded-[2rem] md:rounded-[3.5rem] border-[12px] border-slate-900 bg-slate-950 p-4 perspective-1000">
                <div className="overflow-hidden rounded-[2.5rem] bg-white h-full">
                  <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50 px-6 py-6">
                    ...
                    <motion.div whileHover={{ scale: 1.1 }} className="flex h-10 w-10 items-center justify-center rounded-full bg-blue-600 text-white font-black text-xs shadow-lg shadow-blue-500/30">JD</motion.div>
                  </div>
                  <div className="space-y-6 p-6">
                    ...
                    <motion.div variants={staggerContainer} initial="initial" whileInView="whileInView" className="grid grid-cols-2 gap-4">
                      {[
                        ...
                        { label: "ABV", value: "Rs 3.4k", color: "text-blue-600" }
                      ].map((item, idx) => (
                        <motion.div key={item.label} variants={{ initial: { y: 20, opacity: 0 }, whileInView: { y: 0, opacity: 1 } }} className="rounded-4xl border border-slate-100 bg-slate-50 p-4 hover:border-blue-200 transition-colors">
                    ...
                    <motion.div initial={{ x: 30, opacity: 0 }} whileInView={{ x: 0, opacity: 1 }} transition={{ delay: 1.2, duration: 0.8, ease: "backOut" }} className="luminous-card-dark rounded-[1.75rem] bg-gradient-to-r from-blue-700 to-indigo-700 p-6 text-white shadow-xl shadow-blue-500/20 relative group/alert">
                      <div className="flex items-center gap-3">
                        <motion.div animate={{ rotate: [0, 15, -15, 0] }} transition={{ duration: 0.5, repeat: Infinity, repeatDelay: 3 }}><Bell size={18} className="text-blue-300" /></motion.div>
                        <p className="text-[10px] font-black uppercase tracking-[0.22em] text-blue-200">Push alert</p>
                      </div>
                      <p className="mt-3 text-base font-black leading-tight">Branch 04 Hit target early.</p>
                      <p className="mt-2 text-xs font-medium text-blue-100/80">Approve the next stock push now.</p>
                    </motion.div>
      ...
      <section className="border-y border-slate-200 bg-slate-50/80 py-16 4xl:py-24 px-4 font-inter">
        <Container>
          <motion.div variants={staggerContainer} initial="initial" whileInView="whileInView" viewport={{ once: true }} className="grid gap-6 lg:grid-cols-3">
            {[["Business problem", "When leaders leave the desk, they often lose real-time visibility into what is happening across stores.", "text-rose-500"], ["What Tapito detects", "Tapito detects critical store events, performance swings, and action-worthy alerts and pushes them instantly.", "text-blue-600"], ["What action it enables", "Managers stay decisive on the move with less lag between signal and response.", "text-emerald-600"]].map(([title, text, tone], index) => (
              <motion.div key={title} {...fadeUp(index * 0.1)} {...cardHover} className={`rounded-[1.5rem] md:rounded-[2.5rem] border bg-white p-10 group transition-all h-full ${index === 1 ? "border-blue-200 shadow-[0_45px_100px_-50px_rgba(124,58,237,0.5)]" : "border-slate-200 hover:border-blue-200"}`}>
               ...
               <div className="grid gap-5 md:grid-cols-3 relative z-10">
                {[
                  ...
                  <motion.div key={item.label} variants={fadeUp(0.2 + index * 0.1)} {...cardHover} className="luminous-card luminous-surface-strong rounded-[2.2rem] border border-blue-100 bg-white p-7 text-center group/card">
                    <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-[1.2rem] bg-blue-50 text-blue-600 group-hover/card:bg-blue-600 group-hover/card:text-white transition-all shadow-sm"><item.icon size={26} /></div>
              ...
              <motion.div initial={{ y: 20, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} transition={{ delay: 0.8 }} className="mt-10 rounded-[2.5rem] border border-blue-100 bg-white p-8 relative z-10 shadow-sm cursor-default">
                 ...
                 <div className="mt-8 space-y-4">
                    {[
                      ...
                      <motion.div key={d.t} whileHover={{ scale: 1.02, x: 5 }} className="rounded-4xl bg-slate-50 px-6 py-5 border border-transparent hover:border-blue-200 hover:bg-white transition-all flex items-center justify-between group/q">
                        <div>
        ...
        <Container>
          ...
          <motion.div variants={staggerContainer} initial="initial" whileInView="whileInView" viewport={{ once: true }} className="grid gap-12 md:grid-cols-3">
            {[["24/7 Access", "Constant visibility", "Operational window across all timezone branches"], ["Realtime Alerts", "Zero-lag notification", "Instant push between signal and leader pocket"], ["+30% Speed", "Operational velocity", "Observed increase in decision execution time"]] .map(([value, label, note], index) => (
              <motion.div key={label} {...fadeUp(index * 0.15)} {...cardHover} className="rounded-[2rem] md:rounded-[3.5rem] border border-slate-200 bg-white p-14 text-center group hover:bg-blue-50/40 transition-all shadow-sm">
                <motion.p animate={{ scale: [1, 1.05, 1] }} transition={{ duration: 4, repeat: Infinity }} className="text-5xl font-black tracking-tight text-blue-700 transition-transform group-hover:scale-110">{value}</motion.p>
        ...
        <Container>
          ...
          <div className="grid overflow-hidden rounded-[2rem] md:rounded-[4.5rem] border border-slate-200 shadow-[0_50px_150px_-70px_rgba(15,23,42,0.4)] md:grid-cols-2 relative h-full">
            ...
            <motion.div initial={{ x: 100, opacity: 0 }} whileInView={{ x: 0, opacity: 1 }} viewport={{ once: true }} transition={{ duration: 1 }} className="bg-slate-950 p-14 lg:p-24 text-white relative h-full group">
              ...
              <p className="text-xs font-black uppercase tracking-[0.45em] text-blue-300 relative z-10 font-black">After Tapito Mobile</p>
