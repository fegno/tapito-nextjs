...
export default function SmartAnalyticsPage() {
  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden bg-white pt-24 text-slate-900 selection:bg-purple-100 selection:text-purple-900">
      <InteractiveGrid />
      <div className="pointer-events-none absolute inset-x-0 top-0 h-[400px] lg:h-[560px] bg-[radial-gradient(circle_at_top,rgba(139,92,246,0.18),transparent_38%),radial-gradient(circle_at_20%_20%,rgba(99,102,241,0.14),transparent_24%)]" />
      ...
      <section className="relative py-12 md:py-20 4xl:py-30">
        <Container>
          ...
          <motion.div variants={revealVariant} initial="initial" whileInView="whileInView" viewport={{ once: true, amount: 0.3 }} className="mx-auto mt-16 max-w-5xl rounded-[1.5rem] md:rounded-[2.5rem] border border-purple-200 bg-white/90 p-6 shadow-[0_45px_120px_-65px_rgba(124,58,237,0.55)] backdrop-blur lg:p-8">
                ...
                {[
                  ...
                  <motion.div key={item.label} {...fadeUp(index * 0.1)} {...cardHover} className="luminous-card luminous-surface group rounded-[1.5rem] border border-slate-200 bg-slate-50 p-5">
                    <div className="flex items-center gap-4">
                      <motion.div whileHover={{ scale: 1.1, rotate: 5 }} className="flex h-12 w-12 items-center justify-center rounded-4xl bg-white text-purple-600 shadow-sm transition-shadow group-hover:shadow-md transition-colors group-hover:bg-purple-50"><item.icon size={22} /></motion.div>
                      <div>
              ...
              {[["01", "Data", "Sales, customers, inventory, pricing"], ["02", "AI scoring", "Patterns ranked by confidence and value"], ["03", "Insights", "What changed, where, and why it matters"], ["04", "Recommendations", "The next move your team should execute"]].map(([step, title, text], index) => (
                <div key={step} className="relative group">
                  ...
                  <motion.div variants={fadeUp(index * 0.1)} {...cardHover} className="luminous-card luminous-surface-strong h-full rounded-[2rem] border border-purple-100 bg-white p-8 group-hover:bg-purple-50/30 transition-colors">
                    ...
                    <h3 className="mt-6 text-4xl font-black text-slate-950 transition-colors group-hover:text-purple-700">{title}</h3>
