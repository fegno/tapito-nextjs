...
export default function ProfitabilityEnginePage() {
  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden bg-white pt-24 text-slate-900 selection:bg-blue-100 selection:text-blue-900 font-inter">
      <InteractiveGrid />
      <div className="pointer-events-none absolute inset-x-0 top-0 h-[400px] lg:h-[520px] bg-[radial-gradient(circle_at_top,rgba(59,130,246,0.16),transparent_38%),radial-gradient(circle_at_top_right,rgba(56,189,248,0.12),transparent_24%)]" />
        ...
        <Container>
          <div className="grid items-center gap-8 lg:gap-16 lg:grid-cols-[0.92fr_1.08fr]">
            <div>
              ...
              <motion.div {...fadeUp(0.24)} className="mt-10 flex flex-wrap items-center gap-4">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-xs font-black uppercase tracking-widest text-blue-600">Simulate before spending</span>
                </div>
              </motion.div>
            </div>
            ...
            <motion.div variants={revealVariant} initial="initial" whileInView="whileInView" viewport={{ once: true, amount: 0.3 }} className="rounded-[2.5rem] md:rounded-[3rem] border border-slate-800 bg-slate-950 p-6 shadow-[0_50px_130px_-60px_rgba(15,23,42,0.95)] relative overflow-hidden group">
              <div className="absolute top-0 right-0 w-64 h-64 bg-blue-500/10 blur-[90px] rounded-full pointer-events-none -translate-y-1/2 translate-x-1/2" />
              ...
              <motion.div variants={staggerContainer} initial="initial" whileInView="whileInView" className="mt-6 grid gap-4 md:grid-cols-3 relative z-10">
                {[
                  ...
                  { label: "Base case", value: "Rs 1.31Cr", tone: "text-blue-300" },
      ...
      <section className="border-y border-slate-200 bg-slate-50/80 py-16 4xl:py-24">
        <Container>
          <motion.div variants={staggerContainer} initial="initial" whileInView="whileInView" viewport={{ once: true }} className="grid gap-6 lg:grid-cols-3">
            {[["Business problem", "Retail teams often discover profit pressure only after the month closes, when it is too late to change the outcome.", "text-rose-500"], ["What Tapito detects", "Tapito detects future margin risk by combining seasonality, cost movement, velocity, and modeling.", "text-blue-600"], ["What action it enables", "Leaders can protect profitability earlier and invest with far more confidence.", "text-emerald-600"]].map(([title, text, tone], index) => (
              <motion.div key={title} {...fadeUp(index * 0.1)} {...cardHover} className={`rounded-[1.5rem] md:rounded-[2.5rem] border bg-white p-10 group transition-all ${index === 1 ? "border-blue-200 shadow-[0_40px_90px_-50px_rgba(37,99,235,0.5)]" : "border-slate-200 hover:border-blue-200"}`}>
      ...
      <section className="py-16 4xl:py-24 relative overflow-hidden">
        ...
        <Container>
          ...
          <div className="grid gap-12 lg:grid-cols-[0.8fr_1.2fr] items-center">
            ...
            <motion.div variants={revealVariant} initial="initial" whileInView="whileInView" viewport={{ once: true, amount: 0.14 }} className="rounded-[2.5rem] md:rounded-[3.5rem] border border-blue-200 bg-gradient-to-br from-white via-blue-50/30 to-indigo-50/50 p-10 lg:p-14 relative backdrop-blur-sm group shadow-inner">
              ...
              <motion.div initial={{ y: 20, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} transition={{ delay: 0.6 }} className="mt-8 rounded-[2.5rem] border border-blue-100 bg-white p-8 relative z-10 shadow-sm group-hover:shadow-md transition-shadow">
                ...
                <div className="mt-6 space-y-4">
                  {[
                    ...
                    ["Inventory cut -10%", "Safer cash position, 0.4x stock-out risk", "text-blue-600"],
                  ].map(([title, note, tone], idx) => (
                    <motion.div key={title} initial={{ x: -20, opacity: 0 }} whileInView={{ x: 0, opacity: 1 }} transition={{ delay: 0.8 + idx * 0.1 }} className="rounded-4xl bg-slate-50 px-6 py-5 border border-transparent hover:border-blue-200 hover:bg-white transition-all cursor-pointer">
        ...
        <Container>
          ...
          <div className="grid overflow-hidden rounded-[2.5rem] md:rounded-[3.5rem] border border-slate-200 shadow-[0_50px_140px_-60px_rgba(15,23,42,0.4)] md:grid-cols-2 relative h-full">
            ...
            <motion.div initial={{ x: 100, opacity: 0 }} whileInView={{ x: 0, opacity: 1 }} viewport={{ once: true }} transition={{ duration: 1 }} className="bg-slate-950 p-12 lg:p-20 text-white relative h-full group">
              ...
              <p className="text-xs font-black uppercase tracking-[0.3em] text-blue-300 relative z-10 font-black">After Tapito Profitability</p>
